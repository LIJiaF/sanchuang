...
@section('content')
	...
	<div class="container">
		...
		<div class="con-left">
			...
			<!-- 教改科研板块开始 -->
			<h1 class="title">科技需求&nbsp;&nbsp;&nbsp;<small>信息列表</small></h1>
			
			<div style="min-height:500px;">
			@foreach($needata as $n)
			<p class="content">
				<a href="{{url('need/'.$n->nee_id)}}" title="{{$n->nee_name}}">{{$n->nee_name}}</a>
				<span class="time">{{date('Y-m-d',$n->nee_time)}}</span>
			</p>
			@endforeach
			</div>
			<div class="page">
		...
		<div class="con-right">
			...
			<h1 class="con-right-title"><a href="{{url('admin/need')}}">我要发布需求</a></h1>
			...
			<div class="view">
				...
				<ul>
					<?php $i=1; ?>
					@foreach($neeview as $v)
					<li><span class="block" style="background:#3BC9CB;"><?php echo $i; ?></span><a href="{{url('need/'.$v->nee_id)}}" title="{{$v->nee_name}}">{{$v->nee_name}}</a></li>
					<?php $i++; ?>
					@endforeach
				</ul>
			</div>
			...
			<div class="view">
				...
				<ul>
					<?php $i=1; ?>
					@foreach($neetime as $t)
					<li><span class="block" style="background:#3BC9CB;"><?php echo $i; ?></span><a href="{{url('need/'.$t->nee_id)}}" title="{{$t->nee_name}}">{{$t->nee_name}}</a></li>
					<?php $i++; ?>
					@endforeach
				</ul>
			</div>
